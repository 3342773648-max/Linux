...
| 已实机验证（B2，arm64 实验室） | 在本项目实验环境（Lima arm64，Ubuntu 24.04）完成安装、运行与验收（见 `docs/changes/2026-08-15-b2-real-cluster-acceptance.md`） |
| 已实机验证（B2 x86_64 回归） | 在本项目实验环境（QEMU 模拟 x86_64，Ubuntu 24.04 amd64 双节点）重放同一验收矩阵全部通过（见 `docs/changes/2026-08-15-b2-x8664-regression.md`） |
...
> 注：B2 实验室先后完成 **arm64（aarch64）** 与 **x86_64（amd64）** 两条路径的实机验证；
> x86_64 为 B0 声明的首条路径，回归矩阵（首次部署/verify/二次幂等/reset/重建/重建后幂等）全部通过，
> 见 B2 x86_64 regression change record。
